...
~~**8. Qué pasa con tutorías/registros al desactivar un tutor**~~ — **Resuelto 02-jul-2026.** `SlotView.tutorActive` (derivado de `Tutor.isActive`, sin migración nueva) se expone en el GraphQL `TutoringSlot` y se muestra como badge "Inactiva — sin tutor" en `AdminOfferingDetailPage.tsx` cuando el tutor de ese paralelo perdió el rol. Además: `prepareEnrollment` (inscripción manual) y `syncResponses` (sync de Google Forms) ahora rechazan/omiten nuevas inscripciones a paralelos con tutor inactivo. Al reasignar un tutor activo al paralelo, vuelve a quedar activo automáticamente (no requiere acción extra). Cubierto con tests en `test-offerings.service.enrollment-tutor-active.ts` y `test-response-sync.tutor-active.ts`.
...
- **Botón "Forms activos" ya existe**: en Admin → Tutorías, el botón **"Ver Forms"** (junto a "Generar Form") abre el modal "Formularios Google activos", que lista **todos** los Google Form creados en **todos los semestres** (no solo el semestre seleccionado), con link para editar, sincronizar y eliminar. La query `googleFormLinks` (`offerings.resolver.ts`) no filtra por semestre, así que ningún form generado se pierde de vista — siempre queda accesible ahí aunque cambies de semestre. No hace falta implementar nada nuevo para este caso.
- **Inconsistencia de nota tutor↔admin (02-jul-2026)**: al agregar la nota editable por admin (`adminGrade` promedio, commit `841619d`), `TutorHomePage.tsx` (vista propia del tutor) quedó calculando su "Desempeño semestral" con la fórmula vieja de % ocupación, mostrando un número distinto al que ve el admin en Monitoreo/Tutores para el mismo tutor. Corregido: ahora usa el promedio real de `adminGrade` (mismo cálculo que `getTutorStats`), expuesto vía `myTutoringSlots.adminGrade`. También se limpió un letrero residual en `AdminTutoresPage.tsx` que seguía describiendo la fórmula de ocupación vieja.
